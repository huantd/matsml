"""
Module: Machine Learning Models for Materials Science
Author: Huan Tran (huantd@gmail.com)

This module implements several commonly used machine learning models in
materials science, including:

    - Support Vector Regression (SVR) via scikit-learn
    ...
    - Fully Connected Neural Network (FCNN) via TensorFlow
    - Probabilistic Fully Connected Neural Network (PrFCNN) via
...
class FCNN:
    """
    FCNN: A generic fully connected neural network implemented with TensorFlow.

    Parameters
    ----------
    model_params : dict
        Direct input dictionary specifying the network hyperparameters.
    data_params : dict
        Dictionary produced by ProcessData containing all required data
    ...
    def __init__(self, data_params, model_params):
        ...
        self.batch_size = get_key(
            "batch_size", 
            self.model_params, 
            32
        )

        self.rmse_cv = get_key(
            "rmse_cv", 
            self.model_params, 
            False
        )
    ...
    def check_params(self):
        ...
        if not self.model_file.endswith(".weights.h5"):
            print('    model_file not ends with ".weights.h5", renamed'.ljust(64))
            base = self.model_file.rsplit(".", 1)[0]   # remove old extension
            self.model_file = base + ".weights.h5"
        else:
    ...
    def build_model(self):
        ...
        nn_model = self.keras.models.Sequential()

        # Input layer
        nn_model.add(
            self.keras.layers.InputLayer(
    ...
    def train(self):
        ...
        for train_cv, test_cv in kf:
            ...
            x_cv_train, x_cv_test, y_cv_train, y_cv_test = (
                data_processor.get_cv_datasets(
                    self.train_set, 
                    self.x_cols, 
                    self.y_cols, 
                    train_cv, 
            ...
            rmse_cv_train = np.sqrt(
                mean_squared_error(
                    np.array(y_cv_train).reshape(len(train_cv) * self.y_dim), 
                    y_cv_train_md
                )
            )

            rmse_cv_test = np.sqrt(
                mean_squared_error(
                    np.array(y_cv_test).reshape(len(test_cv)*self.y_dim), 
                    y_cv_test_md
                )
            )
            ...
            if self.rmse_cv:
                ...
                pred_cv_test = pd.concat(
                    [
                        self.train_set.iloc[test_cv][
                            self.id_col
                            + self.sel_cols
                            + self.y_cols
                        ].reset_index(), 
                ...
                unscaled_cv_test = data_processor.invert_scale_y(
                    pred_cv_test, 
                    self.data_dict, 
                    'cv_test'
                )
        ...
        unscaled_train_set = data_processor.invert_scale_y(
            pred_train_set, 
            self.data_dict, 
            'training'
        )
        ...
        if self.n_tests > 0:
            ...
            unscaled_test_set = data_processor.invert_scale_y(
                pred_test_set,
                self.data_dict, 
                'test'
            )

            unscaled_test_set.to_csv(
                'test.csv', 
                index=False
            )
    ...
    def plot(self, pdf_output):
        """
        Plot the train and test predictions and save the figure to the given PDF file.
    
        Parameters
        ----------
        pdf_output : str or pathlib.Path
            Path to the output PDF file where the plots will be saved.
        """
...
class PrFCNN:
    ...
    def build_model(self):
        ...
        # probabilistic layer
        nn_model.add(self.keras.layers.Dense(1+1))

        nn_model.add(
            self.tfp.layers.DistributionLambda(
                lambda t: tfd.Normal(
                    loc=t[..., :1],
                    scale=1e-1 + self.tf.math.softplus(0.1 * t[..., 1:])
                )
            )
        )
        ...
        nn_model.compile(
            loss=negloglik, 
            optimizer=self.keras.optimizers.Adam(learning_rate=0.01)
        )
...
class KRR:
    """
    Kernel Ridge Regression (KRR) model implemented with scikit-learn.

    Parameters
    ----------
    model_params : dict
        Dictionary containing model hyperparameters provided directly by the user.
    data_params : dict
        Dictionary produced by ProcessData containing all data preparation
    ...
    def plot(self, pdf_output):
        ...
        if self.y_scaling in ('logpos', 'logfre'):
            log_scaling = True
        else:
            log_scaling = False
...
class GPR:
    ...
    ----------
    model_params : dict
        Dictionary containing the model hyperparameters provided directly
        by the user.
    data_params : dict
        Dictionary produced by ProcessData containing all data-related
        preprocessing information required for training.
    """
    ...
    def __init__(self, data_params, model_params):
        ...
        self.noise_lb = get_key(
            'noise_lb', 
            self.model_params, 
            0.1
        )

        self.noise_ub = get_key(
            'noise_ub', 
            self.model_params, 
            10
        )
    ...
    def train(self):
        ...
        if self.kernel == "RBF":
            kernel = (
                y_avr**2 * self.RBF(length_scale=1)
                + self.WhiteKernel(
                    noise_level=noise_avr,
            ...
            kernel = (
                y_avr**2 * self.Matern(length_scale=1)
                + self.WhiteKernel(
                    noise_level=noise_avr,
                    noise_level_bounds=(noise_lb, noise_ub)
                )
        ...
        gp = self.GaussianProcessRegressor(
            kernel=kernel, 
            alpha=1e-10, 
            optimizer=self.optimizer, 
        ...
        for train_cv, test_cv in kf:
            ...
            y_cv_train_md = gp.predict(
                x_cv_train, 
                return_std=False
            )

            y_cv_test_md = gp.predict(
                x_cv_test, 
                return_std=False
            )
        ...
        gp_final = self.GaussianProcessRegressor(
            kernel=opt_gp.kernel_, 
            alpha=0,
            optimizer=None
        )
...
class RFR:
    ...
    def __init__(self, data_params, model_params):
        ...
        self.warm_start = get_key(
            'warm_start', 
            self.model_params, 
            True
        )

        self.model_file = get_key(
            'model_file', 
            self.model_params, 
            'model.pkl'
        )

        self.random_state = get_key(
            'random_state', 
            self.model_params, 
            42
        )
    ...
    def train(self):
        ...
        if self.get_feature_importances:
            ...
            feature_importances = [
                ...
                for feature, importance in zip(self.x_cols, importances)
            ]
...
class SVecR:
    """ 
    SVecR: Support Vector Regression with scikit-learn
    ...
    def train(self):
        ...
        print('  ')
        print('  SVecR model trained and saved in "%s"' %
...
class GBR:
    ...
    def train(self):
        ...
        gbr = self.GradientBoostingRegressor(
            random_state=self.random_state,
            loss=self.loss, 
            n_estimators=self.n_estimators,
            criterion=self.criterion
