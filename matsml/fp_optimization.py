...
class FPOptimization:
    ...
    def __init__(self,data_params, optim_params):
        ...
        self.optim_type = get_key('optim_type', self.optim_params, 'gradient_boosting')
        self.optim_fp_file = get_key('optim_fp_file', self.optim_params, 'optim_fp.csv')
        ...
        self.y_train = np.array(self.train_set[self.y_cols[0]]).astype(np.float32)
    ...
    def optimize(self):
        ...
        print ('  Done fingerprint optimization, results saved in %s'%(self.optim_fp_file))
    ...
    def optimize_gb(self):
        ...
        for train_cv,test_cv in kf:
            ...
            idx_up = [ n for n,i in enumerate(feature_importances_cum) if i > 0.99 ][0]
            sel_features = feature_importances_pd.iloc[range(idx_up)]

            # Mearge to the previously selected fingerprints
            sel_xcols = list(set(sel_xcols).union(set(list(sel_features['feature']))))
